...
var projDir, projDirLen = getProjDir()

func getProjDir() (string, int) {
	pathPrefix, err := os.Getwd()
	if err != nil {
		pathPrefix = ""
	}
	if pathPrefix != "" && pathPrefix[len(pathPrefix)-1] != '/' { // 确保路径以 "/" 结尾
		pathPrefix += "/"
	}
	return pathPrefix, len(pathPrefix)
}
...
func makeFuncDetailAndSave(fn *ssa.Function, fs *token.FileSet) {
	// 文件
	file := fs.Position(fn.Pos()).Filename
	if strings.HasPrefix(file, projDir) {
		file = file[projDirLen:] // 相对路径
	}
...
func outputJson(fname string, outputFormat string) {
	Analysis.OptsSetup()
	if e := Analysis.ProcessListArgs(); e != nil {
		log.Fatalf("%v\n", e)
	}
	_, err := Analysis.Render()
	if err != nil {
		log.Fatalf("%v\n", err)
	}
